...
function (thetas, constraint)
{
    ...
    for (j in 1:p) {
        ...
        eta1[[j]] <- if (IRT.param)
            t(outer(Z, bt[-nbt], "-"))
        else matrix(Z, nbt - 1, length(Z), TRUE)
        eta[[j]] <- if (IRT.param)
            bt[nbt] * eta1[[j]]
        else outer(bt[-nbt], bt[nbt] * Z, "+")
        num[[j]] <- exp(apply(eta[[j]], 2, cumsum))
        if (!is.matrix(num[[j]]))
            num[[j]] <- t(num[[j]])
        den[[j]] <- 1 + colSums(num[[j]])
        crf <- rbind(1/den[[j]], num[[j]]/rep(den[[j]], each = ncatg[j] -
            1))
    ...
    for (j in 1:p) {
        if (check.alpha) {
            ...
            I1 <- rbind(0, eta1j) - rep(colSums(num[[j]] * eta1j)/den[[j]],
                each = ncatg[j])
            scores.alpha[j] <- -sum((p.zx * I1[xj, ] * obs) %*%
                GHw, na.rm = TRUE)
        }
        ...
        Part2 <- if (IRT.param)
            -alpha * rowsum(num[[j]][ind1, , drop = FALSE], ind2)
        else rowsum(num[[j]][ind1, , drop = FALSE], ind2)
    ...
    if (!check.alpha) {
        ...
        if (constraint == "gpcm") {
            unlist(mapply(function(x, y) c(x, y), scores.deltas,
                scores.alpha, SIMPLIFY = FALSE, USE.NAMES = FALSE))
        }
        else {
